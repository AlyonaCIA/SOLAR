...
# --- Clustering ---
def perform_kmeans_clustering( # Function name remains the same but now uses MiniBatchKMeans
    data: np.ndarray, n_clusters: int, random_state: int = 42
) -> Tuple[np.ndarray, float]:
    """
    Performs MiniBatch K-Means clustering. # Updated docstring to reflect MiniBatchKMeans
...
def plot_results(
    ...
    clustering_method_name: str = "K-Means" # Added clustering_method_name with default "K-Means"
):
    """
    ...
    for i, (masked_data, channel) in enumerate(
        ...
        if i < num_rows * num_cols:  # Prevent index out of bounds
            ...
            if cluster_pixels_counts and cluster_anomaly_percentages and cluster_index <= len(cluster_pixels_counts):
                cluster_pixels = cluster_pixels_counts[cluster_index-1] # Get pixel count for *current* cluster
                cluster_percentage = cluster_anomaly_percentages[cluster_index-1] # Get anomaly percentage for *current* cluster
                title_text_lines.append(f'Cluster {cluster_index}: {cluster_pixels} Pixels ({cluster_percentage:.2f}%)') # Add line with cluster info

            ax.set_title(
                "\n".join(title_text_lines), # Join lines with newline for multiline title
                color='black',
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Size to resize images to. Ignored if --no_resize is used."
    )
    parser.add_argument(
        "--contamination",
        type=float,
    ...
    masked_data_list = []
    channel_names = []
    image_size_for_processing = args.image_size if not args.no_resize else None # Determine size, None if no_resize
    current_image_size = args.image_size if not args.no_resize else None # Initialize, will be updated if no_resize
    ...
    for channel_dir in channels:
        try:
            ...
            masked_data = preprocess_image(
                data, mask, image_size_for_processing # Pass size, can be None
            )
            masked_data_list.append(masked_data)
            if args.no_resize:
                current_image_size = data.shape[0] # Update to original size if no resize
    ...
    anomaly_scores = detect_anomalies_isolation_forest(
        prepared_data, args.contamination, args.random_state
    )

    # Create a 2D anomaly map (for visualization)
    anomaly_map_2d = np.full((current_image_size, current_image_size), np.nan) # Use current_image_size
    valid_pixel_mask_2d_reshaped = valid_pixel_mask.reshape((current_image_size, current_image_size)) # Use current_image_size
    ...
    for anomaly_threshold in args.anomaly_thresholds:
        ...
        if len(anomaly_intensity_features) > 0:  # Proceed if valid anomalies
            ...
            # Perform MiniBatch K-Means Clustering # Changed comment to reflect MiniBatchKMeans
            cluster_labels, _ = perform_kmeans_clustering( # Function name is still perform_kmeans_clustering but now uses MiniBatchKMeans
                anomaly_intensity_features,
                n_clusters_to_use,
                args.random_state
            )
            ...
            for cluster_index in range(n_clusters_global):
                cluster_pixel_count = np.sum(cluster_mask_global == (cluster_index + 1)) # Count pixels in cluster
                cluster_pixels_counts.append(cluster_pixel_count) # Append count to list
                cluster_percentage = (cluster_pixel_count / anomaly_pixels_count) * 100 if anomaly_pixels_count > 0 else 0
                cluster_anomaly_percentages.append(cluster_percentage) # Append percentage to list
                print(f"  Cluster {cluster_index + 1}: {cluster_pixel_count} pixels"
                      f" ({cluster_percentage:.2f}%) of total anomalies") # Report cluster pixel count and percentage
        ...
        plot_results(
            ...
            clustering_method_name="MiniBatch K-Means" # Added clustering_method_name for plot title
        )
