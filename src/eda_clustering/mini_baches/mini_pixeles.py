...
def plot_results(
    ...
    fig.suptitle(
        ...
        f'Anomaly Threshold: {anomaly_threshold:.2f} | Anomalous Pixels: {anomaly_pixels_count}/{total_pixels} ({anomaly_percentage:.2f}%)',
    ...
    for i, (masked_data, channel) in enumerate(zip(masked_data_list, channel_names)):
        ...
        if cluster_pixels_counts and cluster_anomaly_percentages and cluster_index <= len(cluster_pixels_counts):
            ...
            title_lines.append(f'Cluster {cluster_index+1}: {cluster_pixels} Pixels ({cluster_percentage:.1f}%)') # Shorter percentage format
    ...
    plt.tight_layout(rect=[0, 0, 0.93, 0.95], w_pad=0.1, h_pad=0.1) # Adjusted tight_layout rect and reduced whitespace
    filename = os.path.join(
        output_dir, f"minibatch_kmeans_anomalies_threshold_{anomaly_threshold:.2f}_clusters.png" # More concise filename
    )
    plt.savefig(filename, bbox_inches='tight', dpi=150) # Increased dpi for better quality
    plt.close(fig)
