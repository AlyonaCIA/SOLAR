...
def create_circular_mask(data: np.ndarray, metadata: dict) -> np.ndarray:
    ...
    try:
        ...
        solar_radius_arcsec = metadata.get('RSUN_OBS', metadata.get(
            'R_SUN', 960.0))  # Observed or nominal radius
        ...
        log.debug(f"Disk mask params: Center=({x_center:.2f}, {
                  y_center:.2f}), Radius={solar_radius_pixels:.2f} pix")
    except Exception as e:
        log.error(f"Error reading metadata for mask creation: {
                  e}. Using fallback to image center and min dimension / 2.", exc_info=True)
    ...
    log.debug(f"Generated mask shape: {
              mask.shape}, Masked pixels (inside circle): {np.sum(mask)}")
    return mask


def preprocess_image(
    data: np.ndarray, mask: np.ndarray, target_size: Optional[int] = None
) -> np.ndarray:
    ...
    """
    log.debug(f"Preprocessing single image. Original shape: {
              data.shape}, Target size: {target_size}")

    if data.shape != mask.shape:
        raise ValueError(f"Data shape {data.shape} and mask shape {
                         mask.shape} mismatch.")
    ...
    if target_size is not None:
        ...
        if data.shape == target_shape:
            ...
            resized_data = resize(data.astype(np.float32),
                                  target_shape, mode='reflect', anti_aliasing=True)
            # Mask resize should be order 0 (nearest neighbor) to keep boolean nature
            resized_mask = resize(mask, target_shape, mode='reflect',
    ...
    log.debug(f"Applying mask. Mask shape: {
              resized_mask.shape}, True values: {np.sum(resized_mask)}")
    ...
    log.debug(f"Preprocessing complete for single image. Output shape: {
              masked_data.shape}")
    return masked_data
...
def prepare_data_concatenated(
    ...
    log.info(f"Preparing data for model from {
             len(masked_data_list)} channels. Image shape: {image_shape}")

    # Check that all images have the same shape
    for i, img in enumerate(masked_data_list):
        if img.shape != image_shape:
            raise ValueError(f"Image shape inconsistency: Channel {
                             i} has shape {img.shape}, expected {image_shape}.")
    ...
    reshaped_data = stacked_data.reshape(
        (n_pixels_flattened, n_channels))  # Shape (H*W, C)
    ...
    log.info(f"Found {total_valid_pixels} valid pixels out of {
             n_pixels_flattened} total after removing NaNs.")

    if total_valid_pixels == 0:
        raise ValueError(
            "No valid pixels found after handling NaNs. Cannot proceed with modeling.")
