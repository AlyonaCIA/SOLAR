...
def plot_anomaly_clusters(
    ...
    cluster_pixels_counts: list[int] | None = None,  # Pixels in each cluster
    # % of total anomalies in each cluster
    cluster_anomaly_percentages: list[float] | None = None,
    ...
    """
    if not masked_data_list or not channel_names or len(masked_data_list) != len(channel_names):
    ...
    anomaly_percentage_total = (
        (anomalous_pixels_count / total_pixels_image_grid) * 100 if total_pixels_image_grid > 0 else 0
    )
    suptitle_text = (
    ...
    vmax_global = np.percentile(all_valid_pixels, 98) if len(all_valid_pixels) > 0 else 1
    # Handle cases where percentiles might be the same (flat image) or invalid
    if vmin_global >= vmax_global:
        vmin_global, vmax_global = np.nanmin(all_valid_pixels), np.nanmax(all_valid_pixels)
    ...
    for i, (masked_data, channel) in enumerate(zip(masked_data_list, channel_names)):
        ...
        ax.imshow(
            masked_data,
            cmap="YlOrBr",
            origin="lower",  # Original colormap
            vmin=vmin_global,
            vmax=vmax_global,
            alpha=0.6,
        )  # Use global scaling and alpha

        # Overlay clusters only if there are clusters found
        if n_clusters_found > 0 and cluster_mask_2d.shape == masked_data.shape and np.any(cluster_mask_2d > 0):
            ...
            ax.imshow(
                cluster_overlay_data,
                cmap=cluster_cmap,
                alpha=0.8,
                origin="lower",  # Slightly increased alpha for clusters
                interpolation="nearest",  # Use nearest for masks
                # Set vmin/vmax to span cluster labels (1 to N)
                vmin=1,
                vmax=n_clusters_found,
            )
        elif np.any(cluster_mask_2d > 0) and cluster_mask_2d.shape != masked_data.shape:
            log.warning(
                f"Cluster mask shape {cluster_mask_2d.shape} does not match image shape {
                    masked_data.shape
                } for channel {channel}. Skipping cluster overlay."
            )
        ...
        actual_cluster_ids_in_mask = np.unique(cluster_mask_2d[cluster_mask_2d > 0])  # 1-based IDs
        ...
        if (
            cluster_pixels_counts is not None
            and cluster_anomaly_percentages is not None
            ...
            for patch in cluster_patches:
                try:
                    ...
                    if 0 <= original_label_index < n_clusters_found:
                        ...
                        if pixels > 0:
                            # More concise format
                            title_lines.append(f"C{cluster_id}: {pixels} Pix ({percentage:.1f}%)")
                except Exception as e:
                    log.debug(f"Error processing cluster patch label {patch.get_label()}: {e}")
        ...
        ax.set_title(
            ...
            color="black",
            fontsize=10,
            pad=5,  # Adjusted fontsize and padding again
        )
    ...
    if cluster_patches:  # Only add legend if there are clusters to show
        ...
        valid_patches = []
        if cluster_pixels_counts is not None and len(cluster_pixels_counts) == n_clusters_found:
            for patch in cluster_patches:
                try:
                    cluster_id = int(patch.get_label().split(" ")[1])
                    original_label_index = cluster_id - 1
                    if 0 <= original_label_index < n_clusters_found and cluster_pixels_counts[original_label_index] > 0:
        ...
        if valid_patches:
            fig.legend(
                handles=valid_patches,
                loc="upper right",
                bbox_to_anchor=(0.98, 0.95),  # Position of the legend
                fontsize="small",
                frameon=True,  # Add frame for clarity
                framealpha=0.9,
    ...
    filename = os.path.join(
        ...
        f"{clustering_method_name.lower().replace(' ', '_')}_anomalies_threshold_{anomaly_threshold:.2f}_k{
            n_clusters_found
        }_clusters.png",  # Use method name and found k in filename
    )
    try:
