            ...
            if n_clusters_global > 0:
                for cluster_index in range(1, n_clusters_global + 1):
                    ...
                    ax.imshow(
                        ...
                        alpha=0.7, # Adjusted alpha for clusters too, for balance
...
def main():
    ...
    parser.add_argument(
        "--n_clusters",
        type=int,
        default=5,  # Changed default to 5 clusters (as requested)
    ...
    for anomaly_threshold in args.anomaly_thresholds:
        ...
        if len(anomaly_intensity_features) > 0:  # Proceed if valid anomalies
            ...
            n_clusters_to_use = args.n_clusters # Use n_clusters from argument (now default 5)
