...
def plot_results(
    ...
    filename = os.path.join(
        output_dir, f"kmeans_anomaly_detection_threshold_{anomaly_threshold:.2f}_global_clusters.png" # Reverted to original filename
    )
...
def main():
    ...
    channels_arg =  ['94', '131', '171', '193', '211', '233', '304', '335', '700'] # Specify all 9 channels
    ...
    for anomaly_threshold in args.anomaly_thresholds:
        ...
        print(f"np.sum(anomaly_mask_global) in main loop: {np.sum(anomaly_mask_global) }")
