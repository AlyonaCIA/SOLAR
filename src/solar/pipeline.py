...
import sunpy.map # Import sunpy explicitly if used directly
...
class SolarAnomalyPipeline:
    ...
    def _validate_params(self):
        ...
        if self.cluster_method not in ['kmeans', 'minibatchkmeans']:
            raise ValueError(f"Invalid cluster_method: {self.cluster_method}. "
                             "Choose 'KMeans' or 'MiniBatchKMeans'.")
    ...
    def _load_and_preprocess_all(self) -> None:
            ...
            try:
                ...
                if self._image_shape is None:
                    ...
                    self._total_pixels_final_shape = (self._image_shape[0] *
                                                    self._image_shape[1])
                    self.log.info(f"First image processed. Final shape: "
                ...
                self.log.error(f"Unexpected error processing {channel}: {e}",
                               exc_info=True)
                continue
        ...
        self.log.info(f"Loaded {len(self._masked_data_list)} channels: "
                      f"{self._loaded_channel_names}.")
        ...
        try:
            ...
            (self._prepared_data,
             ...
             self._nan_mask_flat,
             _) = prepare_data_concatenated(self._masked_data_list)
            # --- FIN CORRECCIÓN LLAMADA ---
            self._total_valid_pixels = int(np.sum(self._valid_pixel_mask_flat))
            self.log.info(f"Data prepared. Valid pixels for modeling: "
                          f"{self._total_valid_pixels}")
    ...
    def _cluster_anomalies(
            self, anomaly_features: np.ndarray) -> Tuple[np.ndarray, float]: # Inertia is float
        ...
        if actual_n_clusters < self.n_clusters:
            self.log.warning(f"Reducing n_clusters from {self.n_clusters} to "
                             f"{actual_n_clusters} due to only {n_anomalies} anomalies.")
        elif actual_n_clusters <= 1:
             self.log.warning(f"Only {n_anomalies} anomaly/anomalies found. "
                              "Assigning all to cluster 0. Clustering skipped.")
             # Assign all to cluster 0, return 0 inertia
             return np.zeros(n_anomalies, dtype=int), 0.0


        self.log.info(
            f"Running {self.cluster_method} ({n_anomalies} features, "
            f"k={actual_n_clusters}, state={self.random_state})"
        )
    ...
    def _create_cluster_mask_2d(
        ...
        if anomaly_mask_valid_flat.shape[0] != self._total_valid_pixels:
            raise ValueError(f"Internal mismatch: anomaly_mask_valid_flat len "
        ...
        if cluster_labels.size != n_anomalies_found:
             ...
             if n_anomalies_found > 0 and cluster_labels.size == 0:
                  ...
                  self.log.error(f"Mismatch: {n_anomalies_found} anomalies found, but "
                                 f"cluster_labels array is empty.")
        ...
        original_anomalous_flat_indices = original_valid_flat_indices[
            anomalous_indices_in_valid_set
        ]
        ...
        if n_clusters_found > len(cluster_colors):
            self.log.warning(f"Clusters ({n_clusters_found}) exceed defined colors "
                             f"({len(cluster_colors)}). Using 'tab10'.")
    ...
    def _plot_results(
        ...
        fig, axes = plt.subplots(
            num_rows, num_cols, figsize=(num_cols * 5, num_rows * 4.5),
            dpi=120, squeeze=False
        )
        axes = axes.flatten()

        # Figure Title
        anomaly_perc = (anomalous_pixels_count / self._total_pixels_final_shape * 100
        ...
        all_valid = np.concatenate([
            d[~np.isnan(d)].flatten() for d in self._masked_data_list if np.any(~np.isnan(d))
        ]) if self._masked_data_list else np.array([])
        ...
        for i, (masked_data, channel) in enumerate(
            ...
            ax.imshow(masked_data, cmap='YlOrBr', origin='lower',
                      vmin=vmin_global, vmax=vmax_global, alpha=0.5)
            ...
            if n_clusters_found > 0 and cluster_mask_2d.shape == masked_data.shape:
                ...
                ax.imshow(cluster_overlay, cmap=cluster_cmap, alpha=0.6, origin='lower',
                          interpolation='nearest', vmin=vmin_overlay, vmax=vmax_overlay)
        ...
        if cluster_patches:
            fig.legend(
                handles=cluster_patches, loc='upper right',
                bbox_to_anchor=(0.98, 0.95), fontsize='small',
                frameon=True, framealpha=0.9, title="Anomaly Clusters"
            )
        ...
        filename = os.path.join(
            self.output_dir,
            f"{self.cluster_method}_anomalies_thresh_{anomaly_threshold:.2f}_k{self.n_clusters}.png"
        )
        try:
    ...
    def run(self, anomaly_thresholds: List[float]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.log.info(f"Data processed. Shape: {self._image_shape}, "
                          f"Valid px: {self._total_valid_pixels}")
            ...
            for threshold in sorted(anomaly_thresholds): # Process in order
                ...
                results[threshold] = {
                    ...
                    "anomaly_percentage_of_valid":
                        (anomaly_count / self._total_valid_pixels * 100
                         if self._total_valid_pixels > 0 else 0),
                ...
                cluster_mask_2d, cluster_cmap, cluster_patches, n_clusters_found = \
                    self._create_cluster_mask_2d(anomaly_mask_valid, cluster_labels)
                ...
                if n_clusters_found > 0:
                    ...
                    for cluster_id in cluster_ids_in_mask:
                        if cluster_id == 0: continue # Skip background
