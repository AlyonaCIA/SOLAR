...
def main(cli_args=None):
    ...
    if cli_args:
        ...
        parser.add_argument(
            "--contamination", type=float, default=0.05, help="Estimated proportion of anomalies for Isolation Forest."
        )
        ...
        try:
            ...
            if args.mask_type == "occulting":
                ...
                if args.file_type == "fits":
                    if current_metadata:
                        print(
                            f"      Calling create_occulting_mask_fits with data shape {data.shape if data is not None else 'None'}"
                        )
                        mask = create_occulting_mask_fits(data, current_metadata)
                    else:
                        print(
                            f"    Error: FITS metadata missing for channel {channel_num} for occulting mask. Skipping."
                        )
                        continue
                elif args.file_type == "jp2":
            ...
            if mask is not None:
                print(
                    f"    Preprocessing image for {channel_num}. Data shape: {data.shape if data is not None else 'None'}, Mask shape: {mask.shape if mask is not None else 'None'}"
                )
    ...
    if not masked_data_list:
        print(
            "No data successfully loaded and preprocessed. Exiting."
        )  # This should now only happen if all channels genuinely fail
        if cli_args:
            return None
        return
    ...
    for anomaly_threshold in args.anomaly_thresholds:
        ...
        if total_valid_pixels_resized > 0:  # Ensure we only operate where scores exist
            anomaly_mask_global_2d[valid_score_pixel_mask_2d] = (
                anomaly_map_2d[valid_score_pixel_mask_2d] < anomaly_threshold
            )
        ...
        indices_in_prepared_data_for_clustering = indices_in_prepared_data_for_clustering[
            indices_in_prepared_data_for_clustering != -1
        ]
        ...
        if anomaly_intensity_features.shape[0] > 0:
            ...
            if clusters_to_use > 0:
                ...
                (
                    ...
                    anomaly_mask_global_2d, cluster_labels_for_threshold, valid_pixel_mask_1d_bool, args.image_size
                )
            else:
