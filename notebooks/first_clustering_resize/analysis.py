...
from sklearn.cluster import (
    MeanShift,  # Import clustering tools
    estimate_bandwidth,
)
...
def plot_isolation_forest_anomalies(ax, masked_data, anomaly_mask_channel, channel_name, threshold):
    ...
    ax.imshow(
        np.ma.masked_where(~anomaly_mask_channel, anomaly_mask_channel),
        origin="lower",
...
def plot_clustered_anomalies(
    ...
    ax.set_title(
        f"AIA {channel_name} Å - Clustered Anomalies", color="black", fontsize=12, pad=10
    )  # Title with GLOBAL cluster count
    ax.text(
        0.5,
        -0.18,
        f"Anomaly Threshold: {threshold:.2f}",
...
def plot_comparison_decision(
    ax,
    masked_data_list,
    anomaly_mask_channel,
    cluster_mask_global,
    cluster_cmap_global,
    n_clusters_global,
    cluster_patches_global,
    ...
    ax.set_title(
        f"AIA {channel} Å (Global Clusters: {n_clusters_global})", color="black", fontsize=14, pad=10
    )  # Title with GLOBAL cluster count
    ax.text(
        0.5,
        -0.18,
...
if __name__ == "__main__":
    ...
    for d in all_dirs:
        ...
        print(
            f"Checking directory: {d}, is_dir: {is_dir}, excludes_1600: {excludes_1600}, excludes_1700: {excludes_1700}"
        )  # Detailed check
    ...
    if masked_data_list:  # Check if masked_data_list is not empty before proceeding
        ...
        for anomaly_threshold in anomaly_thresholds:
            ...
            if len(anomaly_pixels_indices_global) > 0:  # Only cluster if there are GLOBAL anomalies
                ...
                for anomaly_pixel_index_2d in anomaly_pixels_indices_global:
                    ...
                    if tuple(anomaly_pixel_index_2d) in pixel_index_map:
                        ...
                        )  # Store valid anomaly pixel indices
                ...
                if (
                    anomaly_intensity_features.size > 0
                ):  # Proceed only if there are valid anomaly pixels after NaN removal
                    # Estimate bandwidth for MeanShift based on INTENSITY FEATURES
                    bandwidth_global = estimate_bandwidth(
                        anomaly_intensity_features,
                        quantile=bandwidth_quantile,
                        n_samples=min(500, len(anomaly_intensity_features)),
                    )  # Use intensity features
                    ...
                    if bandwidth_global > 0:  # Bandwidth must be positive for MeanShift
                        ...
                        print(
                            f"  Global Clustering (Threshold {anomaly_threshold:.2f}, Quantile {
                                bandwidth_quantile
                            }): Estimated {n_clusters_global} clusters, Bandwidth: {bandwidth_global:.2f}"
                        )  # Print bandwidth
                        ...
                        for cluster_idx in range(n_clusters_global):
                            ...
                            if len(cluster_pixels_indices) > 0:
                                ...
                                cluster_mask_global[tuple(cluster_pixels_indices.T)] = cluster_idx + 1
                                cluster_color = cluster_cmap_global(cluster_idx / n_clusters_global)
                                cluster_patches_global.append(
                                    mpatches.Patch(color=cluster_color, label=f"Cluster {cluster_idx + 1}")
                                )
                        ...
                        print(
                            f"  Global Clustering (Threshold {anomaly_threshold:.2f}, Quantile {
                                bandwidth_quantile
                            }): Bandwidth too small, no global clusters estimated."
                        )
                        cluster_cmap_global = matplotlib.colors.ListedColormap(["Reds"])  # Fallback colormap
                        cluster_patches_global.append(
                            mpatches.Patch(color="red", label="Global Anomalies (No Clusters)")
                        )
                else:
                    print(
                        f"  Global Clustering (Threshold {anomaly_threshold:.2f}, Quantile {
                            bandwidth_quantile
                        }): No valid anomaly pixels after NaN removal. No clustering performed."
                    )
                    cluster_patches_global.append(
                        mpatches.Patch(color="none", label="No Valid Global Anomalies for Clustering")
                    )
            ...
            fig.suptitle(
                f"Anomaly Detection in SDO/AIA EUV Channels using Isolation Forest\nAnomaly Threshold: {
                    anomaly_threshold:.2f}\nBandwidth Quantile: {bandwidth_quantile}",
                fontsize=18,
            )  # Main title with threshold and quantile
            ...
            if cluster_patches_global and num_channels < num_rows * num_cols:  # Add legend to the last used subplot
                ...
                cluster_patches_global and num_channels == num_rows * num_cols
            ):  # Add to the very last subplot if all are used
            ...
            filename = os.path.join(
                output_dir,
                f"anomaly_detection_threshold_{anomaly_threshold:.2f}_quantile_{
                    bandwidth_quantile:.2f}_global_clusters.png",
            )
