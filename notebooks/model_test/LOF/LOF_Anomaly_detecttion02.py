...
def detect_anomalies_lof(data: np.ndarray, contamination: float, n_neighbors: int = 20) -> np.ndarray:
...
def plot_results(
    ...
    filename = os.path.join(
        output_dir, f"lof_kmeans_anomaly_detection_threshold_{anomaly_threshold:.2f}_global_clusters.png"
    )
...
def compute_feature_importance(
    anomaly_data: np.ndarray, non_anomaly_data: np.ndarray, channel_names: list, output_dir: str
):
...
def plot_anomaly_distribution(anomaly_scores: np.ndarray, anomaly_threshold: float, output_dir: str):
    ...
    plt.axvline(x=anomaly_threshold, color="red", linestyle="--", label=f"Threshold: {anomaly_threshold:.2f}")

    # Annotate anomaly percentage
    anomaly_percentage = 100 * np.sum(anomaly_scores < anomaly_threshold) / len(anomaly_scores)
    plt.text(anomaly_threshold + 0.1, plt.ylim()[1] * 0.9, f"{anomaly_percentage:.2f}% anomalies", color="red")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="SDO/AIA Anomaly Detection using Local Outlier Factor (LOF) and K-Means Clustering"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
    ...
    parser.add_argument("--contamination", type=float, default=0.01, help="Estimated proportion of anomalies.")
    parser.add_argument(
        "--n_clusters", type=int, default=5, help="Number of clusters for K-Means. If None, use Elbow method."
    )
    parser.add_argument("--max_k", type=int, default=10, help="Maximum number of clusters to test with Elbow method.")
    parser.add_argument("--random_state", type=int, default=42, help="Random seed for reproducibility")
    parser.add_argument("--n_neighbors", type=int, default=20, help="Number of neighbors for LOF algorithm.")
    parser.add_argument(
        "--compute_importance", action="store_true", help="Compute and plot feature importance of each channel."
    )
    ...
    anomaly_scores = detect_anomalies_lof(prepared_data, args.contamination, args.n_neighbors)
    ...
    for anomaly_threshold in args.anomaly_thresholds:
        ...
        elbow_k, silhouette_k, plot_path = plot_elbow_and_silhouette(
            anomaly_intensity_features, max_k=args.max_k, output_dir=args.output_dir
        )
        ...
        if len(anomaly_intensity_features) > 0:  # Proceed if valid anomalies
            ...
            if args.compute_importance:
                ...
                non_anomaly_indices = np.argwhere(
                    non_anomaly_mask & valid_pixel_mask_2d.reshape((args.image_size, args.image_size))
                )
                ...
                if len(non_anomaly_features) > 0:
                    ...
                    if len(non_anomaly_features) > len(anomaly_intensity_features):
                        indices = np.random.choice(
                            len(non_anomaly_features), size=len(anomaly_intensity_features), replace=False
                        )
                        non_anomaly_features = non_anomaly_features[indices]

                    compute_feature_importance(
                        anomaly_intensity_features, non_anomaly_features, channel_names, args.output_dir
                    )
            ...
            cluster_labels, _ = perform_kmeans_clustering(
                anomaly_intensity_features, n_clusters_to_use, random_state=args.random_state
            )
