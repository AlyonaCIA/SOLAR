...
def plot_results(
    ...
    filename = os.path.join(
        output_dir, f"kmeans_anomaly_detection_threshold_{anomaly_threshold:.2f}_global_clusters.png"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="SDO/AIA Anomaly Detection using Isolation Forest and K-Means Clustering"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
    ...
    parser.add_argument(
        "--no_resize", action="store_true", help="Process images at their original size, ignoring --image_size."
    )
    ...
    for channel_dir in channels:
        try:
            ...
            masked_data = preprocess_image(
                data,
                mask,
                image_size_for_processing,  # Pass size, None for no resize
            )
    ...
    anomaly_map_2d = np.full((final_image_size, final_image_size), np.nan)  # Use final image size
    valid_pixel_mask_2d_reshaped = valid_pixel_mask.reshape(
        (final_image_size, final_image_size)
    )  # Use final image size
    ...
    for anomaly_threshold in args.anomaly_thresholds:
        ...
        valid_pixel_mask_2d = ~nan_mask.reshape((final_image_size, final_image_size))  # Use final_image_size
        valid_pixel_indices_2d = np.argwhere(valid_pixel_mask_2d)
        pixel_index_map = {tuple(index_2d): i for i, index_2d in enumerate(valid_pixel_indices_2d)}
