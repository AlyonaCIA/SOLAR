...
def plot_results(
    ...
    anomaly_percentage = (anomaly_pixels_count / total_pixels_resized) * \
        100 if total_pixels_resized > 0 else 0
    fig.suptitle(
        f'{clustering_method_name} Anomaly Clusters in SDO/AIA {file_type.upper()} Channels\n'
        f'Anomaly Threshold: {anomaly_threshold:.2f} | Anomalous Pixels (resized): {
...
def main():
    ...
    parser.add_argument("--data_dir", type=str, required=True, # Make data_dir required
    ...
    parser.add_argument("--file_type", type=str, default="fits", choices=['fits', 'jp2'], # Choice argument
    ...
    parser.add_argument("--jp2_mask_radius", type=int, default=1600, # JP2 specific argument
                        help="Fixed radius in pixels for JP2 mask (used if file_type is 'jp2'). Assumes 4096 original size.")

    # --- Algorithm Parameters ---
    parser.add_argument("--anomaly_thresholds", type=float, nargs='+', default=[-0.1, 0.0, 0.1], # Example thresholds
                        help="Anomaly score threshold(s) for Isolation Forest (lower scores are more anomalous).")
    parser.add_argument("--contamination", type=float, default=0.02, # Adjusted default
                        help="Estimated proportion of anomalies in the dataset for Isolation Forest.")
    parser.add_argument("--n_clusters", type=int, default=5, # Adjusted default
            ...
            if args.file_type == 'fits':
                ...
                if data is not None:
                    ...
                    if data.shape != (4096, 4096) and args.jp2_mask_radius == 1600:
                         ...
                         print(f"Warning: JP2 image shape {data.shape} is not 4096x4096. Adjusting mask radius from {args.jp2_mask_radius} to {scaled_radius}.")
    ...
    print(f"Valid pixel mask (1D) shape: {valid_pixel_mask_1d.shape}, Sum: {np.sum(valid_pixel_mask_1d)}") # (n_total_pixels,)
        ...
        if anomaly_intensity_features.shape[0] > 0 and anomaly_intensity_features.shape[0] >= args.n_clusters :
            ...
            if n_clusters_final > 0:
                for cluster_index in range(1, n_clusters_final + 1):
                    ...
                    print(f"  Cluster {cluster_index}: {cluster_pixel_count} pixels ({cluster_percentage:.2f}% of total anomalies)")
        elif anomaly_intensity_features.shape[0] > 0 :
             print(f"Not enough anomalous samples ({anomaly_intensity_features.shape[0]}) to form {args.n_clusters} clusters. Skipping clustering.")
        else:
