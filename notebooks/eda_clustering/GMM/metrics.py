...
def detect_anomalies_isolation_forest(data: np.ndarray, contamination: float, random_state: int) -> np.ndarray:
    ...
    iso_forest = IsolationForest(
        ...
        random_state=random_state,  # Added random_state
    )
    iso_forest.fit(data)
...
def perform_gmm_clustering_with_search(data: np.ndarray, _: int, random_state: int = 42) -> np.ndarray:
    ...
    try:
        ...
        plt.plot(
            n_components_range, silhouette_scores, marker="o", linestyle="-", label="Silhouette Score", color="green"
        )
...
def main():
    ...
    parser.add_argument(
        "--data_dir", type=str, default="Data/sdo_data", help="Path to directory containing SDO/AIA data."
    )  # Updated data_dir
    parser.add_argument(
        "--output_dir", type=str, default="./output_clustering", help="Directory to save output figures."
    )
    parser.add_argument(
        "--anomaly_thresholds",
        type=float,
    ...
    parser.add_argument(
        "--contamination", type=float, default=0.05, help="Contamination parameter for Isolation Forest."
    )  # Added contamination
    parser.add_argument(
        "--random_state", type=int, default=42, help="Random seed for reproducibility"
    )  # ADDED random_state argument
    ...
    for anomaly_threshold in args.anomaly_thresholds:
        ...
        anomaly_data = reduced_data[
            anomaly_mask_global[valid_pixel_mask.reshape((args.image_size, args.image_size))].flatten()
        ]
        ...
        for method_name, clustering_func in clustering_methods.items():
            ...
            if method_name in ["GMM"]:  # Methods that need n_clusters parameter
                ...
                n_clusters_to_plot = min(len(np.unique(cluster_labels)) - (1 if -1 in cluster_labels else 0), 5)
            else:
            ...
            plot_results(
                ...
                method_name,
                total_pixels,
                anomaly_pixels_count,  # Pass pixel counts
            )
