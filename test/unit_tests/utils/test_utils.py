...
def sample_metadata():
    ...
    return {
        'CRPIX1': 5.5, 'CRPIX2': 5.5, 'CDELT1': 1.0, 'CDELT2': 1.0,
        'CUNIT1': 'arcsec', 'CUNIT2': 'arcsec', 'RSUN_OBS': 4.0,
        'NAXIS1': 10, 'NAXIS2': 10
    }
...
def test_create_circular_mask_valid(sample_image_data, sample_metadata):
    ...
    assert mask[center_y_idx, center_x_idx] == True
...
def test_create_circular_mask_fallback(sample_image_data):
    ...
    assert mask[center_y_fallback, center_x_fallback] == True # Centro (dist 0) <= 960
    ...
    assert mask[0, 0] == True # <-- CORREGIDO: Esperamos True
    # --- FIN DE CORRECCIÓN ---
...
def test_preprocess_image_no_resize(sample_image_data, sample_mask):
    ...
    processed = utils.preprocess_image(sample_image_data, sample_mask,
                                       target_size=None)
    ...
    np.testing.assert_array_equal(processed[sample_mask],
                                  sample_image_data[sample_mask])

def test_preprocess_image_with_resize(sample_image_data, sample_mask):
    """Verifica preprocesamiento con redimensionado."""
    target_size = 5
    processed = utils.preprocess_image(sample_image_data,
                                       sample_mask, target_size=target_size)
    assert isinstance(processed, np.ndarray)
    assert processed.shape == (target_size, target_size)
    resized_mask_manual = resize(sample_mask, (target_size,
                                               target_size),
...
def test_prepare_data_concatenated_success(sample_masked_list_data):
    ...
    scaled_data, valid_mask_flat, nan_mask_flat, image_shape = \
        utils.prepare_data_concatenated(masked_list)
    ...
    expected_nan_mask = np.array([False, False, True, False, True, False,
                                  False, True, False])
