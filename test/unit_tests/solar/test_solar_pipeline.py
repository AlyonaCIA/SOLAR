...
def solar_pipeline_fixture(tmp_path):
    ...
    for channel_dir_name, fits_filename in TEST_FITS_FILES.items():
        ...
        if not asset_file_path.is_file():
             pytest.skip(f"Test FITS file not found: {asset_file_path}. \
...
def test_pipeline_run_successful_execution(solar_pipeline_fixture):
    ...
    assert results.get("status") == "success", f"Pipeline failed \
    ...
    for threshold in thresholds:
        ...
        if anomalies_found and n_clusters_found > 0:
             assert plot_path_str is not None, f"Plot path is None for\
             ...
             print(f"Warning: Plot path {plot_path_str} exists for \
...
def test_pipeline_run_handles_no_data(tmp_path):
    ...
    assert "No data was successfully loaded" in results.get("message", "") or \
           "Pipeline failed due to data or \
