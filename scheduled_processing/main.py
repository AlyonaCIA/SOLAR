...
def process_solar_images(timestamp=None):
    ...
    try:
        ...
        response = requests.post(f"{SOLAR_API_URL}/start-fits-analysis", json={"timestamp": timestamp})
...
def list_processed():
    ...
    try:
        all_blobs = list(
            storage_client.list_blobs(GCS_BUCKET_NAME, prefix=f"{PROCESSED_IMAGES_PREFIX}/", max_results=10)
        )
    ...
    blobs = storage_client.list_blobs(GCS_BUCKET_NAME, prefix=f"{PROCESSED_IMAGES_PREFIX}/", delimiter="/")
    ...
    return jsonify({"status": "success", "count": len(timestamps), "timestamps": timestamps, "debug_info": debug_info})
...
def get_processed_images(timestamp):
    ...
    blobs = list(storage_client.list_blobs(GCS_BUCKET_NAME, prefix=f"{PROCESSED_IMAGES_PREFIX}/{timestamp}/"))

    if not blobs:
        return jsonify({"status": "error", "error": f"No processed images found for timestamp {timestamp}"}), 404
    ...
    return jsonify({"status": "success", "timestamp": timestamp, "images_by_threshold": images_by_channel})
...
def get_latest():
    ...
    debug_info = {
        "raw_blobs": [b.name for b in list(bucket.list_blobs(prefix=f"{PROCESSED_IMAGES_PREFIX}/", max_results=5))]
    }

    # Important: You need to explicitly iterate over the prefixes
    prefixes = []
    blob_iter = storage_client.list_blobs(GCS_BUCKET_NAME, prefix=f"{PROCESSED_IMAGES_PREFIX}/", delimiter="/")
    # Iterate to populate prefixes
    for _ in blob_iter:
        pass
    ...
    if not prefixes:
        return jsonify({"status": "error", "error": "No processed images found", "debug_info": debug_info}), 404
...
def start_processing_route():
    ...
    with processing_lock:
        if timestamp in processing_locks:
            return jsonify({"status": "skipped", "timestamp": timestamp, "reason": "already_processing"})
    ...
    return jsonify(
        {"status": "processing_started", "timestamp": timestamp, "message": "Processing started in background"}
    )
