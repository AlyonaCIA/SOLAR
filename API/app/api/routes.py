...
from app.api.pipeline.executor import run_pipeline, save_and_list_raw_fits
from app.api.pipeline.job_manager import JobStatus, job_manager
...
def save_to_storage(file_path: str, object_name: str) -> dict:
    ...
    if USE_GCS and bucket:
        try:
            ...
            blob.make_public()
            return {"url": blob.public_url, "path": object_name, "storage_type": "gcs"}
        except Exception as e:
    ...
    return {"url": f"file://{os.path.abspath(local_path)}", "path": local_path, "storage_type": "local"}
...
def construct_query(
    ...
    query = Fido.search(time, series, wave, email_attr, sample)
...
def process_fits_analysis(request):
    ...
    SkyCoord(
        center_x * u.arcsec - half_square_size * u.arcsec,
    ...
    for channel in channels:
        try:
            ...
            query_result = client.search(
                a.Time(vso_time_start, vso_time_end), a.Instrument("aia"), a.Wavelength(int(channel) * u.angstrom)
            )
    ...
    if not downloaded_files:
        return {"error": "No FITS files were downloaded.", "logs": download_logs, "status_code": 500}
    ...
    try:
        ...
        return {"error": error_msg, "traceback": tb, "download_logs": download_logs, "status_code": 500}
...
async def start_fits_analysis(request: dict = Body(...)):
    ...
    if "timestamp" not in request:
        return JSONResponse(status_code=400, content={"error": "Timestamp is required"})
...
async def get_job_status(job_id: str):
    ...
    if job["status"] == JobStatus.COMPLETED:
        ...
        if isinstance(result, dict) and "status_code" in result and result.get("error"):
            ...
            return JSONResponse(
                status_code=result["status_code"], content={"error": result["error"], "details": result}
            )
...
async def analyze_fits_channel(request: dict = Body(...)):
    ...
    if not timestamp or not channel:
        return JSONResponse(status_code=400, content={"error": "Both timestamp and channel are required"})
    ...
    # Start the job in the background
    run_in_background(
        process_fits_analysis,
        job_id,
        {"timestamp": timestamp, "channels": [channel], "email": request.get("email", "j.c.g.gomez@astro.uio.no")},
    )
...
async def fits_raw_files(request: dict = Body(...)):
    ...
    try:
        ...
        return JSONResponse(status_code=400, content={"error": f"Invalid timestamp format: {str(e)}"})
    ...
    job_id = job_manager.create_job(
        {"timestamp": timestamp, "channels": channels, "input_dir": input_dir, "operation": "download_only"}
    )
    ...
    async def download_fits_files():
        try:
            ...
            # Complete the job
            job_manager.complete_job(job_id, {"status": "success", "fits_files": fits_files})
