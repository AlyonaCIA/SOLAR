...
def plot_single_channel(
    ...
    try:
        ...
        if np.any(valid_mask):
            ...
            plt.imshow(masked_data, cmap=channel_cmap, vmin=vmin, vmax=vmax, origin="lower", alpha=0.85)
            ...
            if cluster_mask_global is not None and n_clusters_global > 0:
                ...
                plt.imshow(cluster_overlay, cmap=cluster_cmap_global, alpha=0.6, origin="lower")

        else:
            # If all data is NaN, show a minimal blank image
            plt.imshow(np.zeros_like(masked_data), cmap="viridis")
            plt.text(
                masked_data.shape[1] // 2,
                masked_data.shape[0] // 2,
                "No valid data",
                ha="center",
                va="center",
                fontsize=12,
        ...
        if cluster_patches_global and n_clusters_global > 0:
            plt.legend(handles=cluster_patches_global, loc="upper right", fontsize="small", framealpha=0.7)
...
def plot_results(masked_data_list, channel_names, anomaly_map, cluster_mask, threshold, output_dir):
    ...
    try:
        ...
        if cluster_mask is not None and np.any(cluster_mask > 0):
            ...
            for data, channel in zip(masked_data_list, channel_names):
                ...
                plt.axis("off")
                plt.savefig(os.path.join(output_dir, f"channel_{channel}_clusters.png"), bbox_inches="tight", dpi=150)
                plt.close()
