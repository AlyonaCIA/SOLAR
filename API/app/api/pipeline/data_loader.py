...
def create_circular_mask_jp2(data: np.ndarray, fixed_radius_pixels: int) -> np.ndarray:
    ...
    print(
        f"Created JP2 circular mask with radius {fixed_radius_pixels}, shape {mask.shape}, Sum of True: {np.sum(mask)}"
    )
    return mask
            ...
            try:
                ...
                if data is not None:
                    if data.shape != (4096, 4096) and mask_radius == 1600:
                        ...
                        print(
                            f"Warning: JP2 image shape {data.shape} is not 4096x4096. "
