...
def preprocess_image(
    ...
    if size is not None:
        ...
        resized_mask = resize(mask, (size, size), mode='reflect',
...
def evaluate_minibatch_kmeans_clustering(data: np.ndarray, max_k: int = 10, random_state: int = 42,
...
def create_cluster_mask(
    ...
    cluster_patches = [
        mpatches.Patch(color=cluster_cmap(i / n_clusters), label=f'Cluster {i + 1}')
...
def plot_results(
    ...
    fig.suptitle(
        ...
        fontsize=16, y=0.98  # Adjusted suptitle y position slightly up
    )
    ...
    for i, (masked_data, channel) in enumerate(zip(masked_data_list, channel_names)):
        ...
        ax = axes[i]
        ax.imshow(
            masked_data, cmap='YlOrBr', origin='lower',
        ...
        if cluster_pixels_counts and cluster_anomaly_percentages and cluster_index <= len(
            ...
            title_lines.append(
                f'Cluster {cluster_index + 1}: {cluster_pixels} Pixels ({cluster_percentage:.1f}%)')

        ax.set_title(
            "\n".join(title_lines),  # Multiline title
            color='black', fontsize=11, pad=8  # Adjusted fontsize and padding
        )
        ax.axis('off')
    ...
    filename = os.path.join(
        output_dir, f"minibatch_kmeans_anomalies_threshold_{
            anomaly_threshold:.2f}_clusters.png"  # More concise filename
    )
...
def main():
    ...
    parser.add_argument("--no_resize", action='store_true',
                        help="Process images at original size.")

    args = parser.parse_args()
    os.makedirs(args.output_dir, exist_ok=True)

    channels = args.channels if args.channels else [
    ...
    anomaly_map_2d = np.full(
        (final_image_size, final_image_size), np.nan)  # type: ignore
    anomaly_map_2d[valid_pixel_mask.reshape(
        (final_image_size, final_image_size))] = anomaly_scores  # type: ignore
    ...
    for anomaly_threshold in args.anomaly_thresholds:
        ...
        valid_pixel_mask_2d = ~nan_mask.reshape(
            (final_image_size, final_image_size))  # type: ignore
        valid_pixel_indices_2d = np.argwhere(valid_pixel_mask_2d)
        pixel_index_map = {
        ...
        if len(anomaly_intensity_features):
            ...
            cluster_mask_global, cluster_cmap_global, cluster_patches_global, n_clusters_global = create_cluster_mask(
                anomaly_mask_global, cluster_labels, valid_pixel_mask, final_image_size  # type: ignore
            )
        ...
        plot_results(
            ...
            total_pixels, anomaly_pixels_count, cluster_pixels_counts, cluster_anomaly_percentages,
